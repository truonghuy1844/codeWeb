    ...
    public class ProductsController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpsertProduct([FromBody] ProductUpsertDto productDto)
        {
            try
            {
                ...
                if (spExists == 0)
                {
                    return StatusCode(500, new { success = false, message = "Stored procedure không tồn tại" });
                }
                ...
                await _context.Database.ExecuteSqlRawAsync(
                    "EXEC upsert_product @product_id, @name, @name2, @description, @brand_id, @category_id, @uom, " +
                    "@price1, @date_apply1, @price2, @date_apply2, " +
                    "@url_image1, @url_image2, @url_image3, @user_id, @status",
                    parameters);
            ...
            catch (SqlException ex)
            {
                ...
                {
                    2 => "Stored procedure không tồn tại",
                    515 => "Thiếu dữ liệu bắt buộc",
                    547 => "Dữ liệu tham chiếu không hợp lệ (CategoryId hoặc BrandId không tồn tại)",
                    2627 => "Mã sản phẩm đã tồn tại",
                    8152 => "Dữ liệu quá dài cho trường cho phép",
                    _ => ex.Message.Contains("Bạn không phải người bán") ? "Bạn không có quyền thực hiện thao tác này" : ex.Message
                };

                return StatusCode(500, new { success = false, message = errorMessage, sqlError = ex.Number });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new
        ...
        public async Task<IActionResult> DeleteProduct(string id)
        {
            try
            {
                ...
                return rowsAffected > 0
                    ? Ok(new { success = true, message = "Xóa sản phẩm thành công" })
